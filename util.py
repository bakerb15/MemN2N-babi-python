...
def parse_babi_task(data_files, dictionary, include_question, dt=DataType.NUMPY):
    ...
    if dt == DataType.PYTORCH or dt == DataType.PYTORCH_CUDA:
        ...
        Ttype = torch.cuda.LongTensor if USE_CUDA else torch.LongTensor
        story = torch.from_numpy(story).type(Ttype)
        questions = torch.from_numpy(questions).type(Ttype)
        qstory = torch.from_numpy(qstory).type(Ttype)
